...
const getNodePath = (node, allGhostPage) => {
    if (!node.path) {
        return node
    }
    const slugRegex = new RegExp(`${node.path.replace(/\/$/, ``)}$`, `gi`)

    for (let page of allGhostPage.edges) {
...
const serialize = ({ ...sources } = {},{ site, allGhostPage }, mapping) => {
    const nodes = []
    const sourceObject = {}
        ...
        if (mapping[type] && mapping[type].sitemap) {
            ...
            if (currentSource) {
                ...
                currentSource.edges.map(({ node }) => {
                    ...
                    node = getNodePath(node, allGhostPage)
    ...
    const pageNodes = addPageNodes(nodes, allGhostPage.edges, siteUrl)
